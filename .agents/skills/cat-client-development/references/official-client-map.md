# Official CAT C/C++ Client Map

The preparation script checks out only `lib/c` and `lib/cpp` from `https://github.com/dianping/cat.git` into `temp/cat`. Cone-mode sparse checkout also retains repository-root metadata such as the license.
...
Treat source comments and tests as supporting evidence. When they conflict with executed code, follow the code path and document the discrepancy.
