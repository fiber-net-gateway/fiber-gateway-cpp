...
TEST(ProjectRouteSnapshotTest, CompilesStaticAddressesWithJavaHttpHostRules) {
    ...
    EXPECT_EQ(addresses[0].scheme, fiber::access_server::ProxyUpstreamScheme::Http);
    EXPECT_EQ(addresses[0].host, "127.0.0.1");
    EXPECT_EQ(addresses[0].port, 8080);
    EXPECT_EQ(addresses[0].host_header, "127.0.0.1:8080");
    ASSERT_TRUE(addresses[0].ip_address);
    EXPECT_EQ(addresses[1].scheme, fiber::access_server::ProxyUpstreamScheme::Http);
    EXPECT_EQ(addresses[1].host, "backend");
    EXPECT_EQ(addresses[1].port, 80);
    EXPECT_EQ(addresses[1].host_header, "backend");
    EXPECT_EQ(addresses[2].host, ":80");
    EXPECT_EQ(addresses[2].host_header, ":80");
    EXPECT_EQ(addresses[3].scheme, fiber::access_server::ProxyUpstreamScheme::Https);
    EXPECT_EQ(addresses[3].port, 443);
    EXPECT_EQ(addresses[3].host_header, "backend");
