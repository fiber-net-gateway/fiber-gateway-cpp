...
fiber::async::Task<void> send_plain_response(fiber::http::HttpExchange &exchange,
                                             fiber::http::HttpResponseWriter response, int status_code,
...
proxy_over_connection(fiber::http::HttpExchange &exchange, fiber::http::HttpResponseWriter response,
                      fiber::http::HttpBodyPipeReader request_body_reader, const runtime::LocationRuntime &location,
    ...
    if (!prepare_websocket_handshake(websocket)) {
        ...
        co_await send_plain_response(exchange, response, 500, kScriptErrorBody, listener);
        co_return;
    }
    ...
    if (!build_upstream_request_headers(location, exchange, request_headers, resolved_template_values, websocket)) {
        co_await send_plain_response(exchange, response, 502, kBadGatewayBody, listener);
        co_return;
    }
    ...
    if (!send_header_result) {
        ...
        co_await send_plain_response(exchange, response, map_upstream_error_status(send_header_result.error()),
        ...
        while (true) {
            ...
            if (forward_state.should_write(body_bytes)) {
                ...
                if (!write_result) {
                    ...
                    co_await send_plain_response(exchange, response, map_upstream_error_status(write_result.error()),
    ...
    if (upstream_head->status_code == 101) {
        ...
        auto response_header_result = co_await response.send_header({
                .kind = fiber::http::OutgoingHeaderKind::Final,
...
fiber::async::Task<void>
run_proxy_request(fiber::http::HttpExchange &exchange, fiber::http::HttpResponseWriter response,
    ...
    if (!acquired_result) {
        ...
        co_await send_plain_response(exchange, response, map_upstream_error_status(acquired_result.error()),
                                     map_upstream_error_body(acquired_result.error()), listener);
        co_return;
    }
    acquired = std::move(*acquired_result);

    co_await proxy_over_connection(exchange, response, request_body, location, *acquired.conn, listener,
                                   resolved.header_values, resolved.request_target, log_context);
}
...
fiber::async::Task<void>
ProxyHandler::handle(fiber::http::HttpExchange &exchange, fiber::http::HttpResponseWriter response,
                     fiber::http::HttpBodyPipeReader request_body, const runtime::ListenerRuntime &listener,
                     const runtime::LocationRuntime &location,
                     const std::vector<std::pair<std::string_view, std::string_view>> &path_vars,
    ...
    if (!exchange.protocol().empty() && (exchange.method() != fiber::http::HttpMethod::Connect ||
        ...
        co_await send_plain_response(exchange, response, 501, kNotImplementedBody, listener);
        co_return;
    }
    ...
    if (!location.close_on_client_abort) {
        co_await run_proxy_request(exchange, response, request_body, listener, location, path_vars, services,
                                   log_context, *pool_, *dns_, *peer, acquired);
        co_return;
    }
    ...
    auto completed = co_await fiber::async::when_any(
            ...
            [&]() {
                return run_proxy_request(exchange, response, request_body, listener, location, path_vars, services,
                                         log_context, *pool_, *dns_, *peer, acquired)
                        .select();
            });
    if (completed.is<1>()) {
