#ifndef FIBER_NACOS_DISCOVERY_BASIC_SERVICE_DISCOVERY_H
#define FIBER_NACOS_DISCOVERY_BASIC_SERVICE_DISCOVERY_H
...
class BasicServiceDiscovery final : public common::NonCopyable, public common::NonMovable {
public:
    using State = typename StateOps::State;
    using StatePtr = typename StateOps::StatePtr;
    using UpdateResult = typename StateOps::UpdateResult;
    using CreateResult = typename StateOps::CreateResult;
    ...
    struct Observer {
        using UpdateFn = void (*)(void *context, State &state, ServiceKeyView key, bool first_update,
                                  UpdateResult result) noexcept;
        using ClosedFn = void (*)(void *context, ServiceKeyView key) noexcept;
...
private:
    ...
    struct Entry {
        BasicServiceDiscovery *owner = nullptr;
    ...
    public:
        ...
        void reset() noexcept {
            ...
            if (--entry->ref_count == 0) {
                BasicServiceDiscovery::destroy_entry(entry);
            }
        }
...
public:
    ...
    private:
        friend class BasicServiceDiscovery;
    ...
    BasicServiceDiscovery(event::EventLoop &loop, NamingService &naming_service, StateOps ops = {},
                          Observer observer = {}) noexcept :
        loop_(&loop), naming_service_(&naming_service), ops_(std::move(ops)), observer_(observer) {}
    ...
    [[nodiscard]] std::expected<Lease, NamingServiceError> acquire(std::string_view service_name,
        ...
        // subscribe() may synchronously replay a cached value, and observers may
        // re-enter this object while that replay is running.
    ...
    [[nodiscard]] async::Task<void> shutdown() noexcept {
        ...
        if (!stopping_) {
            ...
            while (Entry *entry = entries_.minimum()) {
                EntryPtr hold(entry);
                retire_entry(*entry, ServiceReadyError::Shutdown, false);
            }
        }
        co_return;
    }
...
private:
    ...
    static void destroy_entry(Entry *entry) noexcept {
        ...
        BasicServiceDiscovery *owner = entry->owner;
    ...
    static void on_notify(void *context, const SubscriptionResult<ServiceInfo> &result) noexcept {
        ...
        if (result.kind == ResultKind::Closed) {
            owner->retire_entry(*entry, ServiceReadyError::Closed, true);
            return;
        }
    ...
    void apply(Entry &entry, const std::shared_ptr<const ServiceInfo> &snapshot) noexcept {
        ...
        const bool first_update = entry.phase == EntryPhase::Pending;
        UpdateResult result = [&]() -> UpdateResult {
            if (first_update) {
                CreateResult created = ops_.create(entry.service_name, entry.group, snapshot);
                entry.state = std::move(created.state);
                FIBER_ASSERT(entry.state != nullptr);
                entry.phase = EntryPhase::Ready;
                return std::move(created.result);
            }
            FIBER_ASSERT(entry.state != nullptr);
            return ops_.update(*entry.state, entry.service_name, entry.group, snapshot);
        }();

        if (observer_.on_update != nullptr) {
            observer_.on_update(observer_.context, *entry.state,
                                ServiceKeyView{.service_name = entry.service_name, .group = entry.group}, first_update,
                                result);
        }
        if (first_update) {
    ...
    void release_lease(Entry &entry) noexcept {
        ...
        if (entry.lease_count == 0) {
            retire_entry(entry, stopping_ ? ServiceReadyError::Shutdown : ServiceReadyError::Retired, false);
        }
    }

    void retire_entry(Entry &entry, ServiceReadyError reason, bool notify_closed) noexcept {
        FIBER_ASSERT(loop_->in_loop());
        ...
        if (entry.state != nullptr && !entry.state_retired) {
            ...
            ops_.retire(*entry.state);
        }
        resume_waiters(entry);

        if (notify_closed && observer_.on_closed != nullptr) {
            observer_.on_closed(observer_.context,
                                ServiceKeyView{.service_name = entry.service_name, .group = entry.group});
        }
    }
...
#endif // FIBER_NACOS_DISCOVERY_BASIC_SERVICE_DISCOVERY_H
