...
public:
    Indexer(std::int32_t start, std::int32_t end, std::unique_ptr<Expression> parent,
            std::unique_ptr<Expression> index) :
        MaybeLValue(start, end), parent_(std::move(parent)), index_(std::move(index)) {}

    const Expression *parent() const { return parent_.get(); }

    const Expression *index() const { return index_.get(); }

    std::unique_ptr<Expression> take_parent() { return std::move(parent_); }

    std::unique_ptr<Expression> take_index() { return std::move(index_); }
