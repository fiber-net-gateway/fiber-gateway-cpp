...
constexpr std::uint8_t to_u8(JsTag tag) {
    return static_cast<std::uint8_t>(tag);
}

constexpr std::uint8_t to_u8(JsHeapKind kind) {
    return static_cast<std::uint8_t>(kind);
}

constexpr std::uint8_t to_u8(JsBorrowedEncoding encoding) {
    return static_cast<std::uint8_t>(encoding);
}
...
JsValue js_make_heap_ref(GcHeader *hdr, JsHeapKind kind) {
    return make_heap_value(hdr, kind);
}
...
JsTag js_value_tag(const JsValue &value) {
    return static_cast<JsTag>(value.tag);
}
...
bool js_value_is_heap_ref(const JsValue &value) {
    return js_value_tag(value) == JsTag::HeapRef && value.payload != 0;
}

bool js_value_is_borrowed_string(const JsValue &value) {
    return js_value_tag(value) == JsTag::BorrowedString;
}

bool js_value_is_borrowed_binary(const JsValue &value) {
    return js_value_tag(value) == JsTag::BorrowedBinary;
}

bool js_value_is_undefined(const JsValue &value) {
    return js_value_tag(value) == JsTag::Undefined;
}
...
std::int64_t js_value_int64(const JsValue &value) {
    return std::bit_cast<std::int64_t>(value.payload);
}

double js_value_double(const JsValue &value) {
    return std::bit_cast<double>(value.payload);
}
