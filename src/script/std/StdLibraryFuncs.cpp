...
bool get_u16_string(const JsValue &value, std::u16string &out) {
    ...
    if (!fiber::json::utf8_write_utf16(js_value_native_string(value).data, js_value_native_string(value).len,
                                       out.data(), scan.utf16_len)) {
        return false;
    }
    return true;
}
...
std::string jsonutil_to_string(const JsValue &value) {
    switch (js_value_type(value)) {
        ...
        case JsNodeType::Binary:
            if (js_value_is_borrowed_binary(value)) {
                return std::string(reinterpret_cast<const char *>(js_value_native_binary(value).data),
                                   js_value_native_binary(value).len);
            } else {
                auto *bin = js_value_heap_ptr<const GcBinary>(value);
                if (!bin) {
                    return std::string(kNilText);
...
Library::FunctionResult make_oom_error(ExecutionContext &context) { return make_error(context, "out of memory"); }
...
std::u16string trim_left_repeat(const std::u16string &src, const std::u16string &search) {
    ...
    std::size_t pos = 0;
    while (pos + search.size() <= src.size() && src.compare(pos, search.size(), search) == 0) {
        pos += search.size();
    }
...
std::uint32_t md5_left_rotate(std::uint32_t x, std::uint32_t c) { return (x << c) | (x >> (32 - c)); }
...
std::array<std::uint8_t, 16> md5_digest(const std::uint8_t *data, std::size_t len) {
    ...
    static constexpr std::array<std::uint32_t, 64> s = {7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
    ...
    for (std::size_t offset = 0; offset < buffer.size(); offset += 64) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            w[i] = static_cast<std::uint32_t>(buffer[idx]) | (static_cast<std::uint32_t>(buffer[idx + 1]) << 8) |
...
std::uint32_t sha1_left_rotate(std::uint32_t x, std::uint32_t n) { return (x << n) | (x >> (32 - n)); }
    ...
    for (std::size_t offset = 0; offset < buffer.size(); offset += 64) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            w[i] = (static_cast<std::uint32_t>(buffer[idx]) << 24) |
                   ...
                   (static_cast<std::uint32_t>(buffer[idx + 2]) << 8) | static_cast<std::uint32_t>(buffer[idx + 3]);
        }
        for (int i = 16; i < 80; ++i) {
...
std::uint32_t sha256_rotr(std::uint32_t x, std::uint32_t n) { return (x >> n) | (x << (32 - n)); }
    ...
    FunctionResult call(ExecutionContext &context) override {
        ...
        for (std::size_t i = 1; i < context.arg_count(); ++i) {
            ...
            for (std::size_t j = 0; j < arr->size; ++j) {
                ...
                fiber::json::JsOpResult cmp =
                        fiber::json::js_binary_op(fiber::json::JsBinaryOp::StrictEq, *elem, arg, nullptr);
...
GcString *ensure_heap_string(ScriptRuntime &runtime, const JsValue &value) {
    ...
    if (js_value_type(value) == JsNodeType::String && js_value_is_borrowed_string(value)) {
        auto *str = runtime.alloc_with_gc(
                fiber::json::gc_estimate_utf8_string_bytes(js_value_native_string(value).len), [&]() {
                    return fiber::json::gc_new_string(&runtime.heap(), js_value_native_string(value).data,
                                                      js_value_native_string(value).len);
                });
        return str;
    }
    return nullptr;
...
public:
    FunctionResult call(ExecutionContext &context) override {
        ...
        JsValue out = make_heap_binary_value(context.runtime(), reinterpret_cast<const std::uint8_t *>(text.data()),
                                             text.size());
        if (js_value_type(out) == JsNodeType::Undefined) {
            return make_oom_error(context);
        }
...
public:
    FunctionResult call(ExecutionContext &context) override {
        ...
        if (context.arg_count() > 1 && is_number_type(context.arg_value(1))) {
            std::int64_t ms = to_int64_default(
                    context.arg_value(1),
    ...
    FunctionResult call(ExecutionContext &context) override {
        ...
        while (start <= input.size()) {
            ...
            if (end > start) {
                ...
                if (!key.empty()) {
                    auto *key_str =
                            runtime.alloc_with_gc(fiber::json::gc_estimate_utf8_string_bytes(key.size()), [&]() {
                    ...
                    if (!existing) {
                        ...
                        auto *arr = reinterpret_cast<GcArray *>(js_value_heap_header(array));
                        if (!push_array_value(runtime, arr, *existing) || !push_array_value(runtime, arr, value_val)) {
                            return make_oom_error(context);
                        }
