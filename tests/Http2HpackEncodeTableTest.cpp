...
TEST(Http2HpackEncodeTableTest, ResolveIndexReturnsStaticAndDynamicIndexes) {
    ...
    ASSERT_NE(static_result.entry, nullptr);
    std::uint32_t static_index = 0;
    ASSERT_TRUE(table.resolve_index(static_result.entry, static_index));
    ...
    EXPECT_EQ(table.activate(server_result.entry), Http2HpackEncodeTable::ActivateResult::Activated);
    EXPECT_EQ(table.activate(powered_by_result.entry), Http2HpackEncodeTable::ActivateResult::Activated);

    std::uint32_t server_index = 0;
    std::uint32_t powered_by_index = 0;
    ASSERT_TRUE(table.resolve_index(server_result.entry, server_index));
    ASSERT_TRUE(table.resolve_index(powered_by_result.entry, powered_by_index));
...
TEST(Http2HpackEncodeTableTest, ShrinkEvictsOldestPoliciesAndTracksPendingSizeUpdate) {
    ...
    Http2HpackEncodeTable table;
    ASSERT_TRUE(table.init(catalog, server_result.entry->entry_size + powered_by_result.entry->entry_size));

    EXPECT_EQ(table.activate(server_result.entry), Http2HpackEncodeTable::ActivateResult::Activated);
    EXPECT_EQ(table.activate(powered_by_result.entry), Http2HpackEncodeTable::ActivateResult::Activated);
    EXPECT_EQ(table.active_count(), 2u);

    table.update_max_dynamic_table_size(powered_by_result.entry->entry_size);
    EXPECT_TRUE(table.has_pending_table_size_update());
    EXPECT_EQ(table.pending_dynamic_table_size(), powered_by_result.entry->entry_size);
    EXPECT_EQ(table.active_count(), 1u);
    EXPECT_FALSE(table.is_active(server_result.entry));
    EXPECT_TRUE(table.is_active(powered_by_result.entry));

    std::uint32_t index = 0;
    EXPECT_FALSE(table.resolve_index(server_result.entry, index));
    ASSERT_TRUE(table.resolve_index(powered_by_result.entry, index));
